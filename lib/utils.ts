...
export function transformSiteToHeroContent(site: PayloadSite): HeroContent {
  return {
    ...
    badgeText: site.hero?.badgeText || 'Junte-se à força do SINDESC',
    title: site.hero?.title || 'Fortaleça sua voz. Fortaleça sua categoria.',
    description: site.hero?.description || 'O SINDESC é mais do que uma entidade: é o eco da sua dignidade profissional.',
    ...
    imageAlt: site.hero?.imageAlt || 'Profissional de enfermagem',
  }
}
...
export function transformSiteToFooterContent(site: PayloadSite): FooterContent {
  ...
  return {
    ...
    description: site.footer?.description || 'Representando e defendendo os direitos dos profissionais da enfermagem com força e dedicação.',
