...
export function transformSiteToHeroContent(site: PayloadSite): HeroContent {
  return {
    badge: site.hero?.badge || 'Unidos pela Saúde!',
    badgeText: site.hero?.badgeText || 'Junte-se à força da Enfermagem',
    title: site.hero?.title || 'Fortaleça sua voz. Fortaleça sua categoria.',
    description: site.hero?.description || 'O SINDESC é mais do que uma entidade: é a defesa da sua dignidade profissional na área da saúde.',
    ...
    imageAlt: site.hero?.imageAlt || 'Profissionais de enfermagem e saúde',
  }
}
...
export function transformSiteToFooterContent(site: PayloadSite): FooterContent {
  ...
  return {
    ...
    description: site.footer?.description || 'Representando e defendendo os direitos dos profissionais de enfermagem e saúde com força e dedicação.',
